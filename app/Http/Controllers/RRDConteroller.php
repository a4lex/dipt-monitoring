...
class RRDConteroller extends Controller
{
    const PERIODS = array(
        '1h'  => array(
            'time_format' => 'H:i',
    ...
    function getRRDData (Request $request) {
        ...
        $rrdDir = env('RRD_DIR', '/var/www/rrd');
        ...
        foreach ($request->get('names') as $snmpName) {
            ...
            $rrdDBFile = $rrdDir . '/' . $template->shared . '/' . $template->name . '/' . sprintf("%010d", $request->get('id', 0));
...
//        date_default_timezone_set(C_TIME_ZONE);
